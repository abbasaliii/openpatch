...
| Jul 22 | `v0.17.0` guided repair installation | Replaced the fragile popup permission flow with a persistent full-page installer that revalidates policy, integrity, scope, and live selectors; uses expiring session-only handoff; survives denied permissions; confirms activation on the original tab; and exposes clear cancel, retry, and recovery states |
| Jul 22 | `v0.18.0` unified author return path | Added a local-only author workspace for a patch returned by Codex: the site validates DSL policy and request scope, a path-scoped extension bridge independently revalidates the sender, bytes, clean target, policy, and scope, then opens the exact target and guided installer without making users navigate back or import the file twice |
