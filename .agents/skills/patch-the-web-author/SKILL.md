---
name: patch-the-web-author
description: Inspect a website complaint or missing capability and author, validate, test, and package a domain-scoped Patch the Web repair using the constrained transformation DSL. Use when a user asks Codex to fix, simplify, restyle, reorganize, make accessible, add search, filtering, comparison, keyboard navigation, validation, or safe local persistence to a website they do not control; also use when updating or checking an existing .patch-the-web.json community patch.
---

# Author a Patch the Web repair
...
1. Translate the complaint into observable acceptance criteria.
   ...
   - If the Patch the Web extension supplied a structural preflight, use it only as a privacy-safe starting hint. Reinspect every selector against the live DOM.
...
4. Author the patch.
   - Create `src/registry/patches/<site>-<repair>.patch-the-web.json`.
...
- Preserve the site's real submission and authentication logic. Patch the Web may improve the interface around it but must not impersonate a successful transaction.
